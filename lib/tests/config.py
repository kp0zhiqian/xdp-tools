...
PROJECT_PATH = os.path.abspath(__file__).replace("/lib/tests/config.py", "")

XDP_LOADER_DIRECTORY = PROJECT_PATH + "/xdp-loader"
XDP_FILTER_DIRECTORY = PROJECT_PATH + "/xdp-filter"
XDPDUMP_DIRECTORY = PROJECT_PATH + "/xdp-dump"
...
TEST_PROG_PATH = f"{PROJECT_PATH}/lib/tests/test_progs"

if config['global']['exec_file'] == "complied":
    XDP_FILTER = f"{PROJECT_PATH}/xdp-filter/xdp-filter"
    XDP_LOADER = f"{PROJECT_PATH}/xdp-loader/xdp-loader"
    XDPDUMP = f"{PROJECT_PATH}/xdp-dump/xdpdump"
